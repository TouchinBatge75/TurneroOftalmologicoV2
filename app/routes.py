...
def mover_turno_a_area(
    ...
    """
    Mueve un turno de su área actual hacia otra área.

    Esta función realiza únicamente la operación común
    del Core:

    1. Finaliza el turno actual.
    2. Crea un nuevo TurnoArea.
    3. Genera el número del nuevo turno.
    4. Registra la salida del área anterior.
    5. Registra la entrada al área destino.

    Las validaciones de negocio deben realizarse antes
    de llamar a esta función.

    Esta función NO hace commit.
    El endpoint que la utilice decide cuándo guardar.
    """
    ...
    prioridad_nueva = (
        tipo_prioridad
        if tipo_prioridad is not None
        else (
            turno_actual.tipo_prioridad
            or 'NORMAL'
        )
    )
    ...
    nuevo_turno = TurnoArea(
        ...
        tipo_prioridad=prioridad_nueva,
        estado='ESPERA'
    )
    ...
    historial_salida = HistorialTurno(
        ...
        motivo=(
            ...
            or (
                f'Salida de '
                f'{turno_actual.area.nombre}'
                if turno_actual.area
                else 'Salida del área'
            )
        ),
        usuario=usuario
    )
...
# =====================================================
# COMPLETAR ÁREA PREVIA DE UN SERVICIO
# =====================================================

@bp.route(
    '/turnos/<int:turno_id>/completar-area-previa',
    methods=['POST']
)
def completar_area_previa(turno_id):
    try:
        # =============================================
        # 1. BUSCAR TURNO
        # =============================================
        ...
        if turno.estado != 'EN_ATENCION':
            return jsonify({
                'success': False,
                'error': (
                    'Solo se puede completar el paso previo '
                    'desde un turno que esté en atención'
                )
            }), 409
        ...
        # =============================================
        # 4. EL TURNO DEBE ESTAR LIGADO A UN SERVICIO
        # =============================================
        ...
        # =============================================
        # 5. BUSCAR CONFIGURACIÓN DEL SERVICIO
        # =============================================

        configuracion = obtener_servicio_en_sede(
            atencion.sede_id,
            servicio.id
        )
        ...
        # =============================================
        # 6. VALIDAR ÁREA PREVIA
        # =============================================

        area_previa = (
            configuracion.area_previa
        )
        ...
        if not area_previa:
            return jsonify({
                ...
                'error': (
                    f'El servicio {servicio.nombre} '
                    f'no tiene un área previa configurada'
                )
            }), 409

        if turno.area_id != area_previa.id:
            return jsonify({
                'success': False,
                'error': (
                    'El turno no se encuentra '
                    'en el área previa configurada '
                    'para este servicio'
                )
            }), 409

        # =============================================
        # 7. BUSCAR SERVICIO PENDIENTE
        # =============================================
        ...
        if not atencion_servicio:
            return jsonify({
                'success': False,
                'error': (
                    f'No existe un servicio pendiente '
                    f'{servicio.nombre} '
                    f'para esta atención'
                )
            }), 409

        # =============================================
        # 8. DETERMINAR ÁREA EJECUTORA
        # =============================================
        ...
        # =============================================
        # 11. MOVER AL ÁREA EJECUTORA
        # =============================================

        nuevo_turno = mover_turno_a_area(
            turno_actual=turno,
            area_destino=area_ejecutora,
            servicio_id=servicio.id,
            doctor_id=None,
            tipo_prioridad=turno.tipo_prioridad,

            motivo_salida=(
                f'Paso previo completado '
                f'para {servicio.nombre}'
            ),

            motivo_entrada=(
                f'Pendiente de realizar '
                f'{servicio.nombre}'
            ),
        ...
        return jsonify({
            ...
            'area_previa': {
                'id': area_previa.id,
                'codigo': area_previa.codigo,
                'nombre': area_previa.nombre
            },
...
def trabajo_social_turnos():
    try:
        ...
        # =============================================
        # 3. VALIDAR DISPONIBILIDAD EN LA SEDE
        # =============================================
        ...
        # =============================================
        # 4. OBTENER COLA DESDE EL CORE GENÉRICO
        # =============================================
        ...
        # =============================================
        # 5. AGREGAR DATOS PROPIOS DE TRABAJO SOCIAL
        # =============================================
...
def trabajo_social_afiliar(turno_id):
    try:
        ...
        if (
            not turno.area
            or turno.area.codigo != 'TRABAJO_SOCIAL'
        ):
            return jsonify({
                'success': False,
                'error': (
        ...
        # =============================================
        # 9. HISTORIAL ESPECÍFICO DE AFILIACIÓN
        # =============================================
        ...
        return jsonify({
            ...
            # Se conserva esta clave por compatibilidad
